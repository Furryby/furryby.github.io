window.__startFurryApp = function() {
        ...
        async function tryCenterMapToUserLocation() {
            return await new Promise(resolve => {
                ...
                const applyIpCenter = (lat, lon, cityName = '', force = false) => {
                    if (ipCenterApplied && !force) {
                        return true;
                    }

                    if (!getKnownCityCenter(cityName)) {
                        window.__furryIpGeoDebug = { ok: false, reason: 'no_known_city_match', lat, lon, cityName };
                        return false;
                    }

                    if (!isSafeIpCenter(lat, lon)) {
                        window.__furryIpGeoDebug = { ok: false, reason: 'unsafe_center', lat, lon, cityName };
                        return false;
                    }
                ...
                window[callbackName] = data => {
                    ...
                    const cityName = String(city && (city.name_ru || city.name_en || city.name) || '');
                    const lat = Number(
                    ...
                    window.__furryIpGeoDebug = { ok: true, lat, lon, cityName, data };
                    const applied = applyIpCenter(lat, lon, cityName);
                    if (typeof map.isStyleLoaded === 'function' && !map.isStyleLoaded()) {
                        map.once('style.load', () => applyIpCenter(lat, lon, cityName, true));
                        setTimeout(() => applyIpCenter(lat, lon, cityName, true), 500);
                    }
                    finish(applied);
                };
        ...
        function isSafeIpCenter(lat, lon) {
            lat = Number(lat);
            lon = Number(lon);
            if (!Number.isFinite(lat) || !Number.isFinite(lon)) {
                return false;
            ...
            return lat >= 35 && lat <= 75 && lon >= 15 && lon <= 180;
        }

        function resetMapToDefaultCity() {
            if (!map || hasProfileOpenParams()) {
                return;
            }
            map.jumpTo({
                center: [37.6173, 55.7558],
        ...
        function getKnownCityCenter(cityName) {
            const wanted = normalizeCityKey(cityName);
            if (!wanted || !citiesData || !allData.length) {
                return null;
            }

            let indexes = citiesData[cityName] || null;
            if (!indexes) {
                const matchedCity = Object.keys(citiesData).find(city => normalizeCityKey(city) === wanted);
                indexes = matchedCity ? citiesData[matchedCity] : null;
            }
            ...
            indexes.forEach(index => {
                ...
                const lat = Number(form && form.lat);
                const lon = Number(form && form.lon);
                if (Number.isFinite(lat) && Number.isFinite(lon)) {
                    latSum += lat;
                    lonSum += lon;
                    count++;
        ...
        let sidebarBatchSize = 100;
        let sidebarVisibleCount = 100;
        ...
        const MARKERS_DATA_VERSION = String(Date.now());
        const MARKERS_CACHE_NAME = 'furry-markers-cache-v1';
        const MARKERS_CACHE_URL = './markers-lite.json?v=' + MARKERS_DATA_VERSION;
        const MARKERS_CACHE_FALLBACK_URL = './github/markers-lite.json?v=' + MARKERS_DATA_VERSION;
        const PUBLIC_FORMS_URLS = ['./forms.public.json?v=' + MARKERS_DATA_VERSION, './github/forms.public.json?v=' + MARKERS_DATA_VERSION];
        ...
        function createProfileHTML(form, index, threshold) {
            const vipBadge = form.is_vip ? ' <span class="vip">★</span>' : '';
            const cleanText = stripHtml(form.form_text);
            ...
            let html = '<div class="profile-item' + (isNew ? ' new' : '') + '" onclick="toggleProfile(' + index + ')">';
        ...
        function extractTelegramUsername(value) {
            const raw = String(value ?? '').trim();
            const tgMatch = raw.match(/^tg:\/\/user\?id=(\d+)$/);
            if (tgMatch) {
                return 'user?id=' + tgMatch[1];
            }
        ...
        function createProfileAvatarMarkup(form) {
            ...
            const iconSvg = isVk
                ? '<svg viewBox="0 0 24 24" fill="#2787F5" focusable="false"><path d="M12.8 17.8c-5.5 0-8.7-3.8-8.8-10.1h2.8c.1 4.6 2.1 6.6 3.7 7V7.7h2.6v4c1.6-.2 3.2-2 3.8-4h2.6c-.4 2.5-2.3 4.4-3.6 5.2 1.3.7 3.4 2.3 4.2 4.9h-2.9c-.7-1.9-2.2-3.4-4.1-3.7v3.7h-.3z"></path></svg>'
        ...
        function createTelegramWriteLink(form, className = 'profile-link') {
            const rawLink = form && form.user_link;
            const safeUrl = safeTelegramUrl(rawLink);
            if (!safeUrl) return '';
            const username = extractTelegramUsername(rawLink);
            const isTgLink = /^tg:\/\/user\?id=\d+$/.test(rawLink);
            const dataTag = username ? ' data-telegram-tag="@' + escapeHtml(username) + '"' : '';
            return '<a href="' + safeUrl + '" target="_blank" rel="noopener noreferrer" class="' + className + '"' + (isTgLink ? ' data-tg-user="1"' : '') + dataTag + ' onclick="return handleTelegramWriteClick(event, this)">Написать</a>';
        }
        ...
        function getPhotoCandidates(value) {
            ...
            if (fileName) {
                ...
                candidates.push('photo/' + fileName);
            }
        ...
        function buildPhotoOnError(candidates) {
            ...
            parts.push("this.onerror=null;this.style.display='none';");
            return parts.join(' ');
        }
        ...
        function createProfilePhotoMarkup(form) {
            ...
            return '<div class="profile-photo-wrap"><div class="profile-photo-loading">Загрузка фото...</div><img class="profile-photo" src="' + safeSrc + '" alt="' + safeAlt + '" loading="lazy" onload="' + safeOnLoad + '" onerror="' + safeOnError + '"></div>';
        }
...
async function loadPublicFormsData() {
    setLoadingStatus('Загрузка анкет из API...');
    const payload = await fetchJsonWithTimeout(
    ...
    return validateMarkersPayload(payload.profiles);

    setLoadingStatus('Загрузка меток...');
    const markerUrls = [MARKERS_CACHE_URL, MARKERS_CACHE_FALLBACK_URL];

    if ('caches' in window) {
        const cache = await caches.open(MARKERS_CACHE_NAME);
        try {
            const response = await fetchFirstAvailable(markerUrls, { cache: 'no-store' }, 'Не удалось загрузить лёгкую базу меток');
            const responseForCache = response.clone();
            const payload = validateMarkersPayload(await response.json());
            cache.put(markerUrls[0], responseForCache).catch(error => {
                console.warn('Кеш меток не обновлён:', error);
            });
            return payload;
        } catch (error) {
            const cached = await matchFirstCached(cache, markerUrls);
            if (cached) {
                setLoadingStatus('Загрузка меток из кеша...');
                return validateMarkersPayload(await cached.json());
            }
            throw error;
        }
    }
    ...
    const response = await fetchFirstAvailable(markerUrls, { cache: 'no-store' }, 'Не удалось загрузить лёгкую базу меток');
    return validateMarkersPayload(await response.json());
}
...
async function loadFormsData() {
    ...
    return await loadEncryptedFormsData();
        ...
        function hasProfileOpenParams() {
            const params = new URLSearchParams(location.search);
            return params.has('profile') || params.has('form_id') || params.has('id')
        ...
        async function initApp() {
            try {
                ...
                if (!hasProfileOpenParams()) {
                    Promise.resolve(tryCenterMapToUserLocation()).then(applied => {
                        if (!applied) {
                            resetMapToDefaultCity();
                        }
                    }).catch(error => {
                        console.warn('IP centering skipped:', error);
                        resetMapToDefaultCity();
                    });
                }
                updateHeroMetrics();
                ...
                document.getElementById('search-input').addEventListener('input', searchCity);
                map.on('movestart', () => {
                    isMapInteracting = true;
                    setMapMovingVisualState(true);
                    window.clearTimeout(tileCachePurgeTimer);
        ...
        function findNearestKnownCity(lat, lon) {
            ...
            for (const form of allData || []) {
                const city = String(form && form.settlement || '').trim();
                if (!city || city === 'Точка на карте' || seenCities.has(city)) {
                    continue;
                }
                const formLat = Number(form && form.lat);
                const formLon = Number(form && form.lon);
                if (!Number.isFinite(formLat) || !Number.isFinite(formLon)) {
                    continue;
                }
                const distance = getDistanceMeters(pointLat, pointLon, formLat, formLon);
            ...
            return bestDistance <= 100000 ? bestCity : '';
        }
        ...
        async function copyFurryBazaTelegramLink(event) {
            ...
            try {
                ...
                showToast('Telegram в России не работает. Воспользуйтесь приложением Telegram. Ссылка скопирована');
            } catch (error) {
                showToast('Скопируйте вручную: https://t.me/furbaza');
            }
        ...
        async function handleTelegramWriteClick(event, link) {
            ...
            if (href.startsWith('tg://user?id=')) {
                ...
                try {
                    ...
                    showToast('Скопируйте вручную: ' + href);
                }
                return false;
            }
        ...
        document.addEventListener('DOMContentLoaded', () => {
            const addProfileButton = document.querySelector('.add-profile-btn');
            if (addProfileButton) {
                addProfileButton.addEventListener('click', handleAddProfileClick);
            }
        });
